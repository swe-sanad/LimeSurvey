...
class ConditionsAction extends SurveyCommonAction
{
    ...
    protected function copyConditions(array $args)
    {
        ...
        if (!is_array($copyconditionsto) || count($copyconditionsto) === 0) {
            throw new CHttpException(400, gT('Invalid request: No target question selected.'));
        }

        // Validate that source conditions are selected
        if (!is_array($copyconditionsfrom) || count($copyconditionsfrom) === 0) {
            throw new CHttpException(400, gT('Invalid request: No conditions selected to copy.'));
        }
